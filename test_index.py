...
targets = ["aea.net", "fvcc", "montana.edu"]
found_indices = {t: [] for t in targets}
...
for idx, row in enumerate(grid_data):
    ...
    for cell in row_cells:
        for t in targets:
            if 'hyperlink' in cell and cell['hyperlink'] and t in cell['hyperlink'].lower():
                found_indices[t].append(idx)
            if 'formattedValue' in cell and cell['formattedValue'] and t in cell['formattedValue'].lower():
                found_indices[t].append(idx)

for k, v in found_indices.items():
    print(f"Target '{k}' found on indices: {sorted(list(set(v)))}")
